...
transformed data {
  ...
  vector[N_1] u_sumZ_1_1;    // Sum of Z_1_1 for each group
  ...
  u_sumZ_1_1 = rep_vector(0.0, N_1);

  for (n in 1:N) {
    u_count[J_1[n]] += 1;
    u_sumY[J_1[n]] += Y[n];
    u_sumX[J_1[n], ] += X[n, ];
    u_sumZ_1_1[J_1[n]] += Z_1_1[n]; // Sum Z_1_1 for each group
  }
}
...
parameters {
  ...
  real gamma;                // Coefficient for the group-level predictor
}
...
model {
  ...
  lprior += normal_lpdf(gamma | 0, 1); // Prior for the group-level predictor coefficient
  ...
  for (g in 1:N_1) {
    Syy_adjusted -= 2 * (u[g] +  u_sumZ_1_1[g]) * u_sumY[g] 
                    - (u[g] +  u_sumZ_1_1[g])^2 * u_count[g];
    Syx_adjusted -= (u[g] +  u_sumZ_1_1[g]) * u_sumX[g, ];
  }
